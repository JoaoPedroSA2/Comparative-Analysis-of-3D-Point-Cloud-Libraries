...
def carregar_cpu_o3d(caminho):
    ...
    for _ in range(15):
        ...
        cpu_porcentagem.append(cpu_media)
        cpu_tempo.append(cpu_total)

    print(f"Tempo médio: {statistics.mean(tempos):.5f}s")
    print(f"Tempo mediano: {statistics.median(tempos):.5f}s")

    print(f"CPU média (%): {statistics.mean(cpu_porcentagem):.2f}%")
    print(f"CPU mediana (%): {statistics.median(cpu_porcentagem):.2f}%")

    print(f"CPU tempo médio: {statistics.mean(cpu_tempo):.5f}s")

    return statistics.mean(tempos), statistics.mean(cpu_porcentagem), statistics.mean(cpu_tempo)

def downsample_cpu_o3d(caminho):
    process = psutil.Process(os.getpid())
    num_cpus = psutil.cpu_count()
    ...
    return statistics.mean(tempos), statistics.mean(cpus_percent), statistics.mean(cpus_time)

def normais_cpu_o3d(caminho):
    process = psutil.Process(os.getpid())
    num_cpus = psutil.cpu_count()
